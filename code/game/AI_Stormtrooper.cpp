...
static qboolean NPC_ST_InvestigateEvent( int eventID, bool extraSuspicious )
{
	...
	*/
	...
	if ( level.alertEvents[eventID].level > AEL_MINOR && NPCInfo->investigateCount > 1 && (NPCInfo->scriptFlags&SCF_CHASE_ENEMIES) )
	{
		...
		{//we were able to bMove the investigateGoal to a point in which our bbox would fit
...
void ST_Commander( void )
{
	...
	{//dissolve the group
		...
		for ( i = 0; i < group->numGroup; i++ )
		{
			...
			if ( !(NPCInfo->scriptFlags&SCF_CHASE_ENEMIES) )
			{//not allowed to bMove on my own
				continue;
			}
...
//							cpFlags |= (CP_CLEAR|CP_COVER);	// NOPE, Can't See The Enemy, So Find A New Combat Point
		...
		{//we want to run to a combat point
			...
			{//found a combat point
				...
				//okay, try a bMove right now to see if we can even get there
				if ( (cpFlags&CP_FLANK) )
				{
					if ( group->numGroup > 1 )
					{
...
void NPC_BSST_Attack( void )
{
	...
	{//don't bMove when doing silly look around thing
		bMove = qfalse;
	}
	if ( bMove )
	{//bMove toward goal
	...
	if ( //!TIMER_Done( NPC, "flee" ) ||
		(bMove&&!TIMER_Done( NPC, "runBackwardsDebounce" )) )
	{//running away
		faceEnemy = qfalse;
	}
